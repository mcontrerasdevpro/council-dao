...
⚠️ WARNING ⚠️: The CID is public and permanent (IPFS + on-chain, immutable, forever). Do not put personal data (names, email, phone). See Hard rules.
...
On-chain requires one answer per question, voters must grant location permission to cast a valid vote. The geo dimension is a soft signal (device location), not a cryptographic proof.
